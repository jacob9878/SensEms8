...
(function($) {
	...
	$.formatDate = function(date, pattern) {
		...
		while (pattern.length > 0) {
			...
			if (matched) {
				result.push($.formatDate.patternValue[matched[0]].call(this,
						date));
	...
	};// end-of-function(date, pattern)

	$.formatDate.patternParts = /^(yy(yy)?|M(M(M(M)?)?)?|d(d)?|EEE(E)?|a|H(H)?|h(h)?|m(m)?|s(s)?|S)/;

	$.formatDate.monthNames = [ 'January', 'February', 'March', 'April', 'May',
			'June', 'July', 'August', 'September', 'October', 'November',
			'December' ];

	$.formatDate.dayNames = [ 'Sunday', 'Monday', 'Tuesday', 'Wednesday',
	...
	}; // end-of-$.formatDate.patternValue
