...
import com.imoxion.sensems.server.nio.smtp.handler.ImSmtpSslServerHandler;
...
public class ImSmtpSslInitializier extends ChannelInitializer<SocketChannel> {
...
//        cp.addLast(new WriteTimeoutHandler(ImSmtpConfig.getInstance().getRWTime()/1000));
        // Insert the SslHandler to the ChannelPipeline, startTls flag set to true
        cp.addLast("sslHandler", sslContext.newHandler(socketChannel.alloc()));
        ...
        cp.addLast("basehandler", new ImSmtpSslServerHandler(new ImSmtpSession(sslContext)));
